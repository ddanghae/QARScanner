...
export const CONFIG = {
  ...
  earlyDetect: {
    ...
    // 시간 만료 667건 중 41% 가 +1R 을 찍고 되돌아왔다(4R 도달은 0%).
    // 같은 거래에 청산만 갈아 끼운 결과:
    //   (a) 4R 고정          평균 +0.146R · 승률 37% · PF 1.32 · MDD -37.6R
    //   (b) 1R 절반+본전이동 평균 +0.089R · 승률 49% · PF 1.24 · MDD -19.5R   <- 채택
    //   (c) 2R 후 1R 트레일  평균 +0.055R · 승률 40% · PF 1.13 · MDD -30.4R
    //   (d) 30봉 무반응 탈출 평균 +0.069R · 승률 35% · PF 1.18 · MDD -38.1R
    //
    // (b) 는 평균 수익이 (a) 보다 낮다. 그런데도 채택한 이유는 낙폭 대비다 —
    // 낙폭 1R 당 수익이 (a) 0.0039 vs (b) 0.0046 이고, 승률 37%→49%, MDD 는 절반이다.
    // 점수 70+ 구간에서 특히 갈린다: (a) +0.240R/MDD -14.9R vs (b) +0.208R/MDD -6.7R.
    // 최대 수익을 원하면 partialFrac 을 0 으로 두면 (a) 로 돌아간다.
    //
    // 주의 — 첫 측정에서 (b) 가 +0.290R 로 나왔던 것은 스크립트 버그였다(시간 만료 거래에
    // 부분 익절 성립 여부를 안 보고 +0.5R 을 무조건 얹었다). 지금 값이 수정 후 값이며
  ...
  // 후보끼리 같이 움직이는지 판정. 4시간봉 42봉 = 7일 수익률의 피어슨 상관.
  // 0.7 은 "사실상 같은 베팅" 으로 볼 만한 선 — 낮추면 배지가 전부에 붙어 정보가 없어진다.
  correlation: { bars: 42, threshold: 0.7 },

  // 청산가 추정용 유지증거금률 %. 바이낸스 USDT 무기한은 명목가 구간별로 0.4~5% 이고
  // 소액 구간이 0.4% 다. 여기선 보수적으로 0.5% 하나로 잡는다.
  // ponytail: 구간표를 넣지 않는다 — 경고를 띄울지 말지를 가르는 용도라 자릿수만 맞으면 된다.
  // 실제 청산가는 거래소 계산기로 확인할 것.
