...
순차 검색합니다. 결과는 같은 화면에서 모드별로 묶이며 각 결과 줄에
`급락 반등`·`조기 포착`·`급등 후 급락` 출처가 표시됩니다. 상단의 출처 탭을 누르면
선택한 스캐너 결과만 바로 볼 수 있습니다. 같은 종목이 여러
모드에 잡히면 전략별 방향·점수·계획이 다르므로 합치지 않고 각각 표시합니다.
세 모드는 검색 시작 시각을 공통 기준으로 사용합니다. 한 모드의 일부 자료가 실패하면
해당 섹션에 표시하고, 모든 자료 요청이 실패한 경우에는 후보 0개가 아닌 오류로 알립니다.

- **급락 반등**(기본) — 위 파이프라인. 크게 떨어진 것의 되돌림을 노립니다.
- **조기 포착** — 크게 오르기 전 구간을 노립니다. 4시간봉에서 **14일 추세 강도 +
  24시간 변동 + 최근 상장**으로 채점해 **관찰 → 조건 2개 충족 → 돌파** 3단계로 분류합니다.
  ...
  >
  > **과거 점수 구간별 적중률** — 검증 기간 2026-06-08~2026-07-11, 17,597행에서
  > 70점+ 29%, 55~69 16%, 40~54 12% (같은 기간 전체 3.57%)였습니다.
  > 화면의 "과거 적중률"이 이 숫자입니다. 미래 확률이 아니며 다른 시장에서도 같다는
  > 보장은 없습니다. 조건이 발동해도 대부분은 급등하지 않습니다.
...
- **급등 후 급락** (`pump_fade`, SHORT 전용) — 1시간 마감봉 기준 6시간 +12% 또는
  ...
  **1 과열 감시 → 2 고점 거절 → 3 급락 확인**으로 분류합니다. 고점 대비 12% 이상
  ...
  15분 신호 시각과 맞는 마지막 5분봉이 없거나 최근 5분봉이 비거나 중복되면 결과를 만들지 않습니다.

## 사용법

1. 모바일 하단의 **3개 스캐너 한 번에 검색**을 누르면 세 모드가 순서대로 실행됩니다.
   PC에서는 검색 설정에서 **전체 스캔 (3개 모드)** 을 고른 뒤 검색할 수도 있습니다.
2. 진행률 바에서 단계(종목 수집 → 유동성 필터 → 1차 분석 → 정밀 분석)를 확인합니다.
3. **먼저 볼 후보**와 결과 목록에서 스캐너 출처·점수·단계·방향을 확인합니다.
   먼저 볼 후보는 해당 스캐너 안의 1위이며, 서로 기준이 다른 세 점수를 섞어 비교하지 않습니다.
4. **상세**를 누르면 점수 근거, 시간봉별 상태, 진입·손절·목표가를 봅니다.
5. **TV / TradingView에서 타점 확인**을 누르면 새 탭에서 차트가 열립니다.
6. 모바일은 **검색 설정**을 눌러 방향·최소 점수·거래대금·단계·정렬을 펼친 뒤
   **필터 적용**을 누릅니다. PC에서는 검색 설정이 바로 보입니다.
...
8. **가상 기록**은 검증된 LONG 계산만 지원합니다. SHORT·진행 중 봉·잘못된 가격 계획은
   저장하지 않으며, 이전 형식 기록은 삭제하지 않고 통계에서만 제외합니다. 판정은 기록 뒤
   새로 시작한 4시간봉부터라 첫 최대 4시간이 빠질 수 있고, 금액에는 수수료·실제 체결가 차이가
   포함되지 않습니다. 더 정밀한 장기 기록기는 별도 V2 범위입니다.

### 리페인트(미래 참조) 방지

기본 점수는 **마감 캔들**만 사용합니다. 진행 중 캔들을 포함한 실시간 예상 신호를 보려면
설정에서 별도로 켤 수 있으며, 이 경우 신호는 실시간 추정으로 구분됩니다.
세 모드를 한 번에 검색할 때는 같은 기준 시각을 지키기 위해 진행 중 봉을 자동으로 제외합니다.
피봇은 오른쪽 확인 봉이 닫힌 뒤부터만 사용하고, 구조 돌파는 실제 돌파 봉의 마감 시각에 기록합니다.
기존 계산은 `tests/repaint.test.js`, pump_fade는 미래 5분봉 차단·연속성·prefix 불변성
테스트로 검증합니다.
...
현재 자동 테스트가 전부 통과합니다. (기존 지표·구조·early·페이퍼 기록 회귀와
...
├── css/style.css         # 모바일 우선, 다크모드, 평면 목록/테이블 반응형
├── js/
│   ├── main.js           # 진입점
