...
Binance USDⓈ-M Futures에서 급락 반등, 상승 전 조기 포착, 급등 후 하락 전환 후보를 좁히는 스캐너.
...
3. 선택한 모드에 따라 1시간 또는 4시간 마감봉으로 1차 후보를 걸러냅니다.
4. 남은 후보를 해당 모드의 4시간·1시간·15분·5분 규칙으로 정밀 분석합니다.
5. 시장구조(BOS/CHoCH), 유동성 스윕, FVG·오더블록, 거래량/Delta, 매도 흡수를
   ...
   **0~5단계**(근거 부족→관찰 초기→유동성 회수→구조전환→진입 구간→늦음·추격 금지),
   조기 포착 모드는 **1~3단계**, 급등 후 급락 모드는
   **1 과열 감시 → 2 고점 거절 → 3 급락 확인**을 사용합니다.
6. 후보를 클릭하면 로그인된 TradingView 차트로 연결합니다.

급락 반등 모드는 **롱**(급락·과매도 반등), **숏**(급등·과매수 반락), **양방향**을 지원합니다.
숏은 롱의 대칭 — 고점 유동성 스윕·하락 구조전환·bearish FVG/OB·매수 흡수로 평가합니다.
양방향은 종목별로 롱/숏 중 높은 점수 쪽을 표시합니다. 조기 포착은 LONG 전용,
급등 후 급락은 SHORT 전용이며 전용 모드에서 방향 선택은 잠깁니다.
...
- pump_fade 점수는 급등 뒤 소진·거절·하락 확인 규칙의 충족 합계입니다. 초기 가중치와
  45점 컷은 **실험값**이며 단계·성공 확률·기대수익률로 환산하지 않습니다.
...
- **급등 후 급락** (`pump_fade`, SHORT 전용) — 1시간 마감봉 기준 6시간 +12% 또는
  24시간 +25% 급등을 먼저 확인합니다. 그 뒤 15분 거래량 클라이맥스·긴 윗꼬리 또는
  고점 sweep 실패·Taker Buy 소진·EMA20/VWAP 하향 이탈과 5분 구조 붕괴를 순서대로
  평가합니다. 고점에서 이미 12% 이상 하락했다면 늦은 숏 위험으로 25점을 감점하고,
  최근 고점과 ATR을 이용한 손절이 진입가에서 8% 이상 멀면 계획을 무효 처리합니다.
  임계값과 가중치는 백테스트 완료 전 **EXPERIMENTAL**입니다.
...
5. reversal 결과의 **TV 정합**은 해당 심볼의 15분 TradingView 차트를 열어 Pine v3.4의
   독립 차트 정합을 확인합니다. early와 pump_fade의 **TV 차트**는 별도 관찰용입니다.
   QAR의 방향·점수·단계는 Pine으로 전달되지 않으며, 링크는 심볼과 15분봉만 넘깁니다.
6. 필터 영역에서 방향·점수·거래대금·진행 단계·정렬을 조정하고 **필터 적용**을 누릅니다.
   reversal은 `최소 셋업 점수`를 직접 사용합니다. early는 이 선택기를 숨기고 채점 강도에
   대응하는 실제 `조기 포착 품질 컷`을 표시합니다. pump_fade는 수동 점수와 강도 선택을
   잠그고 검증 전 고정 `45+` 실험 컷을 표시합니다.
...
모든 지표는 과거 데이터만으로 계산되며 미래 데이터를 참조하지 않습니다.
기존 지표는 `tests/repaint.test.js`, pump_fade는 미래 5분봉 차단과 prefix 불변성 테스트로 검증합니다.
...
연구 입력은 심볼별 1h/15m/5m 캔들을 담은 JSON 배열, `{ "records": [...] }`, 또는
JSONL입니다. 운영 가중치를 바꾸지 않고 6시간 12/15%와 24시간 25/30% 조합을 모두 보고합니다.
...
결과는 시간순 60/20/20 분리와 6시간 purge를 사용하며 signal count, hit/base rate, lift,
평균 수익률, MFE, MAE, profit factor, max drawdown을 냅니다. 같은 5분봉에서 -8% 목표와
+5% 손절을 모두 건드리면 순서를 추정하지 않고 `AMBIGUOUS`, 중간·마지막 봉이 부족하면
`INCOMPLETE`로 제외합니다. 수수료·펀딩·슬리피지·체결 제약을 포함하지 않은 총액 기준입니다.
...
├── research/
│   ├── pump-fade-backtest.mjs      # 시간순 threshold sweep CLI
│   └── pump-fade-research-core.mjs # 사후 라벨·성과 요약 순수 함수
