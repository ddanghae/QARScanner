...
- **조기 포착** — 크게 오르기 전 구간을 노립니다. 4시간봉에서 **넓은 변동 구간(박스 폭) +
  레인지 하단권 + 미결제약정(OI) 증가**를 보고 **바닥권 → 반등 시작 → 돌파** 3단계로
  분류합니다. 중형 중심(거래대금 5M↑, 상위 200, 대형코인 제외), 롱 전용. OI·펀딩비는
  공개 엔드포인트를 쓰며 펀딩비는 스캔당 1회만 호출합니다. 이 모드에서는 방향·노이즈
  필터를 자동으로 우회합니다.

  > 이 기준은 감이 아니라 백테스트로 정했습니다. 172종목 83일에서 "+50% / 5일 내" 급등
  > 82건을 찾아 급등 직전 시점으로 되감아 재생한 결과, 원래 기준(변동성 압축·거래량 고갈·
  > 박스 상단 근접)은 포착률 1.2%로 대조군(3.6%)보다 낮았습니다 — 방향이 반대였습니다.
  > 실측된 급등 직전 모습(박스폭 중앙 43% vs 대조 18%, 레인지 위치 0.20 vs 0.40)으로
  > 교체한 뒤 포착률은 37.8%가 됐고 오탐은 3.1%로 오히려 줄었습니다.
  >
  > **다만 향상도의 대부분은 "변동성이 큰 종목 고르기"에서 나옵니다.** 무필터 기준
  > 적중률은 10~16% 수준이라 발동해도 대부분은 급등하지 않습니다. 언제 오를지는 알 수 없는
  > **후보 좁히기** 도구이며 단독 매매 근거로 쓰지 마세요.
  >
