...
9. **급등 후 "지지 vs 급락" 분석 → 대부분 반영 안 함(음성 결과)** + 계획·게이트 실측 보정
   - **질문**: 급등한 코인 중 버티는 것과 반납하는 것을 사전에 구분할 수 있나?
   - **방법**: +30% 급등 141건의 고점 이후 5일을 추적해 되돌림 비율을 구하고,
     급등 "직전" 지표들과의 스피어만 순위상관을 봤다(작은 그룹 비교는 표본이 5건뿐이라 폐기).
   - **되돌림 실태**: 중앙 **82%** 반납, 31%는 **전량 반납**, 절반 미만만 반납(지지)은 **10.6%**.
   - **결정적 모순**: 박스 폭은 급등 확률을 올리면서(3.35x) 되돌림도 올린다(r=+0.23).
     거래대금만 반대 방향(r=-0.24). 즉 스캐너가 "잘 오르지만 잘 반납하는" 종목을 상위에 둔다.
   - **시도했다 되돌린 것**: 거래대금 10점을 채점에 넣어 봤으나, 게이트 통과분 55건으로 검증하니
     상위 N 되돌림 중앙값 차이가 +8/-8/-4/+9/0%p 로 전부 노이즈였다(상위 10 종목도 순서만 바뀜).
     게이트가 이미 소형·고변동만 남겨 그 안에선 거래대금 편차가 작기 때문. 포착률만
     37.8%→33.3% 로 깎여서 **되돌렸다**. → **급등 후 지지 여부는 현재 사전 지표로 구분 불가**가 결론.
   - **대신 반영한 것 (측정 근거가 확실한 것만)**:
     - `boxWidthMinPct: 30 → 50` — 구간별 실측 향상도가 30~50 은 **1.23x(사실상 무신호)**,
       50~80 2.93x, 80~120 **4.39x**, 150~200 3.09x. 표본 5,929.
       결과: 포착률 37.8%→22.6%, 대조군 오탐 3.1%→1.0%, **변별비 12.2x → 22.6x**.
     - `boxWidthMaxPct: 400 → 200` (200 이상은 표본 없음).
     - **목표가·손절을 박스 기준 → ATR 기준으로 교체**. 박스 상단까지 거리 중앙값은 구간별
       36~762% 인데 실제 5일 최대 상승률 중앙값은 8~22% 라 박스 상단은 어디서도 닿지 않았다.
       이제 TP는 진입 +1.5/3/5 ATR(TP3는 박스 상단이 더 가까우면 그쪽).
     - **손절 클램프 버그 수정** — 폭락 후 바닥 종목은 박스 하단이 폭락 저점이라 손절이
       **-70~-88%** 로 나왔다. ATR 이 가격의 38% 인 종목에선 "2 ATR" 이 제약이 못 돼서
       절대 상한 `maxStopPct: 0.20` 을 추가했다. 라이브 확인: 전 후보 -5.9~-20%, RR 1:1.7~5.8.
     - 상세 패널에 되돌림 실측치 안내문 추가(분할 익절 전제로 보라는 문구).

## 검증 상태

- **테스트 97/97 통과** — `node tests/run.js` (indicators, structure, liquidity, scoring,
...
cd QARScanner
node tests/run.js          # 테스트 확인 (97/97 나와야 정상)
