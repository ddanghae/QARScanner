...
18. **모바일 우선 평면형 화면 개편 (2026-08-25)** — 아이폰 12 mini 375×812를
    기준으로, 화면을 가득 채우던 카드 묶음을 선과 여백 중심 구조로 바꿨다.
    - 모바일은 `검색 상태 → 먼저 볼 후보 → 스캐너별 탭 → 줄 목록 → 하단 재검색` 순서다.
      검색 설정은 기본으로 접고 필요할 때만 펼친다. PC는 설정과 표를 바로 볼 수 있다.
    - 전체·반등·조기·급락 탭에 각각 결과 수를 표시하고, 누른 스캐너의 결과와 그 안의
      1위 후보만 보여준다. 세 점수를 섞어 전역 순위를 만들지 않는다.
    - 하단의 **3개 스캐너 한 번에 검색**은 현재 선택과 관계없이 전체 모드로 바꾼 뒤
      기존 통합 검색을 실행한다. 기존 단일 모드 검색과 상세·관심 종목·가상 기록은 유지했다.
    - 강조 후보 한 곳만 진한 면으로 표시하고, 모바일 결과 카드는 줄 목록으로 바꿨다.
      출처·방향·단계·가격 계획 경고는 숨기지 않았다.
    - `resultModeCounts` 회귀 테스트를 추가했고 앱 셸 변경에 따라 서비스워커를 v12로 올렸다.
    - 실제 공개 자료로 세 모드 검색, 출처 탭, 상세 열기/닫기, 설정 열기와 화면 이동을
      375×812 및 1440×1024에서 확인했다. 가로 넘침과 브라우저 오류는 없었다.
      기준 이미지와 최종 화면 비교는 `design-qa.md`에 남겼고 결과는 `passed`다.

## 검증 상태

- **현재 테스트 191/191 통과** — `node tests/run.js`. 기존 계산 회귀와 함께
  API 중단·차단, 검색 중단/재시작, 구조 시점, 5분봉 정렬, 계획 검사, 가상 기록 경계를 확인한다.
- **기존 로컬 화면 검수 통과** — 데스크톱·390×844 모바일에서 세 모드 결과, 한 모드 실패,
  무효 계획 기록 차단, 상세창 키보드, 과거자료 한계, 모바일 가상 기록 표와 브라우저
  `190/190`을 확인했다.
- **모바일 개편 화면 검수 통과** — 375×812 및 1440×1024에서 실제 세 모드 검색,
  출처 탭, 상세, 검색 설정, 설정 화면 이동, 가로 넘침 0px, 브라우저 오류 0건을 확인했다.
...
cd QARScanner
node tests/run.js          # 테스트 확인 (191/191 나와야 정상)
...
8. **모바일 전체 메뉴** — 지금은 900px 미만에서 큰 사이드바를 숨기고 머리글의
   `개요/설정` 전환만 남겼다. 관심 종목·자동 갱신 같은 보조 메뉴까지 모바일에서
   한곳에 모으려면 별도 메뉴 화면이나 드로어가 필요하다.
