...
## 2026-09-19 조기포착 안전성·근거 추적 보완

- 목록의 가격을 마지막 4시간봉 종가가 아닌 24시간 티커의 스캔 시세로 바꾸고, 실제로 산출하지 않던 `6h 0%` 표기를 24시간 변동으로 교체했다. 상세에는 별도의 `signalPrice`(4시간 마감 기준가), 두 가격의 차이, 신호 만료 시각을 보존한다.
- 3단계 `임박`은 24시간 양의 회복과 양의 14일 추세 또는 EMA200 재탈환을 함께 요구한다. 하락 중인데 준비도만 높은 후보는 2단계에 머문다.
- ATR 손절이 0 이하, 25% 초과, 또는 시장가가 4시간 기준가에서 8% 초과 이탈이면 `계획 보류`로 fail-closed 처리한다. 다음 4시간 마감 뒤에는 잔존 계획·기록을 막고 재스캔을 요구한다.
- 50개 정밀 후보는 존재하지 않는 `squeezePct`가 아니라 최종 점수와 같은 입력의 `priority`로 고른다. 화면 기본 필터를 공유한 상위 3개만 CRT + TBS와 첫 눌림을 함께 확인한다.
- 페이퍼 저널은 schema v3: 신호/시장 가격과 시각, 점수 세부 근거, 잠재력·준비도·위험도, CRT/TBS·첫 눌림 상태와 유효성을 기록한다. schema v1~v2 localStorage 기록은 그대로 읽는다.
- 회귀 테스트 224/224 통과 및 실제 Binance 스캔(524 → 194 → 50 → 3, CRT + TBS 확인 3/3)에서 스캔 시세·계획 보류·상세 기준가를 확인했다.

## 2026-09-19 조기포착 단일 스캐너 개편

- 제품 실행 모드를 `early` 하나로 고정했다. 과거 `all/reversal/pump_fade/sweep_retest` 저장값은 자동으로 `early`와 LONG으로 마이그레이션한다.
- 급락 반등과 급등 후 하락은 화면·실행 목록에서 제거했다. 기존 코어와 연구 테스트는 이전 결과 비교를 위해 당분간 보존한다.
- 기존 검증 점수는 `급등 잠재력`으로 유지하고, 현재 방향·EMA200·박스 위치·거래량·ATR·돌파를 설명하는 `준비도`, 하락·추격 위험을 설명하는 `위험도`를 별도 축으로 추가했다. 두 새 축은 확률이나 정렬 점수가 아니다.
- 단계를 `1 관찰 → 2 준비 → 3 임박 → 4 확인 후보 → 5 추격 금지`로 바꿨다. 하락 폭만 큰 후보는 상승 회복 전까지 임박으로 올리지 않는다.
- 첫 눌림 순서 판정과 CRT + TBS는 잠재력 상위 후보에만 추가 확인으로 실행해 1h/15m/5m API 호출을 후보 제한으로 유지한다.
- 상세와 목록에 3축, 첫 눌림 확인, 추격 위험을 표시하고 의사결정 체크리스트가 높은 위험과 5단계를 차단하도록 보완했다.
...
### 2026-09-17 리빌딩 3단계 — 인터페이스 단순화

- 사이드바의 중복 실행·필터 버튼을 제거하고 스캐너/설정 중심으로 단순화. 상단 상태 카드는 시장국면·스캔 상태·검색 범위·표시 후보 4개로 축약.
- 기본 스캔 설정은 모드·방향·최소 점수·거래대금만 노출하고, 자금·단계·특수 필터는 접이식 `세부 필터 · 자금 설정`으로 이동.
- 데스크톱 후보 표를 14열에서 7개 의미 묶음으로 재구성. 모바일 카드는 시장·24h 전망·계획·예상 손익을 분리해 위험 정보를 유지하면서 가독성 개선.
- 모바일 상세의 시간봉 표 너비를 보정해 페이지·상세 카드 가로 넘침 제거. PWA 캐시 `v16`으로 갱신.
- 분석·점수·후보 선정 로직은 변경하지 않음. 자동 회귀 테스트 206/206, 실제 reversal 스캔, 설정 탭·상세 패널, 390px 모바일 및 브라우저 콘솔 오류 0건 확인.

### 2026-09-17 리빌딩 2단계 — 의사결정 체크리스트 + 저널 운영성

- `js/core/decision-gate.js`: 계획 유효성, 단계, 진행 봉, BTC 시장국면, 24시간 전망, CRT, 후보 상관을 `검토 후보/확인 대기/리스크 높음`으로 설명. 기존 점수·정렬·후보 수에는 미반영.
- 데스크톱 표와 모바일 카드에 상태 배지, 상세 패널에 항목별 통과/주의/차단/정보 체크리스트 추가.
- Forward Paper Ledger에 평균 계획 손익비, 종료된 승패 기준 최대 낙폭, 판정/진행/모호 표본 수 및 JSON 백업·CSV 내보내기 추가.
- 기존 localStorage 스키마 호환 유지. 과거 백테스트·모델 성능 검증·전략 임계값 변경은 이 단계 범위에서 제외.
- 자동 회귀 테스트 206/206 및 변경 파일 구문 검사 통과. 실제 reversal 스캔에서 상태 배지·상세 체크리스트·내보내기 버튼 표시, 브라우저 콘솔 오류 0건, 390px 가로 넘침 없음 확인.

### 2026-09-17 리빌딩 1단계 — 시장국면 + Forward Paper Ledger

- `js/core/market-regime.js`: BTC 4시간 마감봉의 EMA20/50, 7일 흐름, ATR 상대 변동성으로 상승·하락·횡보 및 변동성 국면 산출.
- 검증 전 국면값으로 점수를 조작하지 않고 후보별 `시장 흐름 우호/중립/역행` 설명과 기록 스냅샷에만 사용.
- `js/ui/paper.js`를 스키마 v2로 확장. 기존 localStorage 기록 호환, LONG/SHORT 대칭 판정, 동일 봉 손절/목표 모호성 분리, 왕복비용 반영.
- 신호 당시 전략·방향·점수·단계·24시간 전망·시장국면·계획을 고정하고 1/3/6/24시간 실제 경로와 `closeTs`, MFE/MAE를 보존.
- 앱 표시 이름을 `QAR Scenario Scanner`로 변경하고 시장국면 상태 카드와 후보 정합성 배지 추가.
- 자동 테스트 200/200 통과. 실제 Binance reversal 스캔, 브라우저 콘솔 오류 0건, 390px 모바일 카드 전환 및 가로 넘침 없음 확인.

### 2026-09-16 로컬 연구 환경

- `research/lab/` + 루트 `lab.cmd`: 공개 5분봉 수집/압축 저장/품질 검사, UTC 시간봉 집계, 기존 계산 함수 재사용, 시간순 분할·보유기간 purge, 비용/부분청산/모호한 봉 처리, HTML·CSV·JSONL 성적표.
- 기존 세 전략과 같은 방향 CRT 결합 및 CRT 단독을 비교. 선택 종목별 조건 실험으로 전체 시장 순위나 계좌 수익률을 재현하지 않음.
- 관측 원본과 사후 결과를 분리하는 `paper-snapshot`/`paper-settle`. 관측 이후 첫 5분봉 시가 기준. 자동 반복과 실주문은 없음.
- 자료/실험/모의 기록은 Git 제외된 `lab-data` / `lab-runs` / `lab-paper`. 실행마다 코드·설정·데이터 체크섬 보존.
- 초기 BTC/ETH 60일, 총 34,560봉 수집. 소표본 동작 검증이며 성능 우열 판단 금지. 원래 대형 코인 제외 때문에 early 0건 정상.
- 연구 테스트 14개 및 기존 테스트 194개 통과. 사용법은 `research/lab/README.md`.

### 2026-09-16 지표 신뢰성 보완

- 시각 기반 마감 봉 선별 및 마감 경계를 넘은 진행 봉 캐시 폐기.
- 방향 모델은 항상 마감 4h 사용. OHLC·시간 연속성·BTC 마감 시각 일치·시세 최신성 검사.
- CRT와 방향 확률 기한 경과 시 목록·필터·열린 상세 자동 갱신. 조기 전략 승률을 공통 손익 화면에 적용하던 설명 제거.
- 194/194 테스트, 구문 검사, 실제 BTC/ETH API 확률 산출, 브라우저 자동 만료 검증 통과.
- `docs/INDICATOR_REVIEW_2026-09-16.md`에 발견 사항과 미완료 연구 과제 구분. 모델 재학습/CRT 승률 검증을 완료했다고 해석하지 말 것.

### 추가: CRT + TBS 후보 확인

- `js/core/crt-tbs.js`: 최근 마감 4h 범위 + 5m 몸통 이탈/복귀/전환. 마감 시각·데이터 연속성 검사, 양방향 이탈·추격·만료·무효화 처리.
- 모든 모드의 기존 분석 후보에 추가하며 심볼 중복 제거/캐시 재사용. 기존 점수와 24h 모델은 변경하지 않음.
- 목록 배지와 `CRT + TBS 확인만`, 상세의 별도 손절·손실률·목표·비용 반영 손익비. 기록 버튼은 원래 전략의 계획 유지.
- 187/187 테스트 통과. 실제 reversal 스캔과 가상 확인/충돌 UI를 브라우저에서 검증. `tests/crt-tbs-preview.html`은 가상 데이터 UI 검증용.
- 아직 CRT 조건의 수익성/승률 백테스트는 없음. 구체적 규칙은 `docs/CRT_TBS.md`.
